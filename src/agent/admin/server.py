...
<body><main>
  ...
  <section class="grid">
    ...
    <article class="card"><div style="display:flex;justify-content:space-between;gap:12px"><div><h2>内容数据源</h2><div class="meta" style="margin:-10px 0 14px">首次启动会自动注册默认内容中心；每个来源的 URL 均可独立编辑。</div></div><button onclick="openSource()">+ 添加来源</button></div><div class="sources" id="sources"></div></article>
...
async function api(path,opt={}){const r=await fetch(path,{headers:{'Content-Type':'application/json','X-Local-Agent-Action':'confirmed'},...opt}),raw=await r.text();let data=null;if(raw){try{data=JSON.parse(raw)}catch{data=raw}}if(!r.ok){const detail=data&&typeof data==='object'&&'detail'in data?data.detail:data,code=detail&&typeof detail==='object'?detail.code:null,message=typeof detail==='string'?detail:(detail&&detail.message)||raw||`HTTP ${r.status}`;throw new Error(code?`[${code}] ${message}`:message)}return data}
